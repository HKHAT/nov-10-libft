NAME = libft.a

SRCS = ft_atoi.c ft_isalnum.c ft_isdigit.c  ft_memcpy.c ft_putchar_fd.c  \
       ft_striteri.c ft_strlcpy.c ft_strncmp.c ft_tolower.c ft_putchar_fd.c  ft_itoa.c ft_split.c\
       ft_bzero.c ft_isalpha.c ft_isprint.c ft_memchr.c ft_memmove.c ft_putnbr_fd.c ft_putstr_fd.c ft_strchr.c \
       ft_strjoin.c ft_strlen.c ft_strrchr.c ft_toupper.c ft_calloc.c ft_strdup.c ft_strrchr.c \
       ft_calloc.c ft_isascii.c ft_isspace.c ft_memcmp.c ft_memset.c ft_putstr_fd.c ft_putendl_fd.c ft_strdup.c \
       ft_strlcat.c ft_strmapi.c ft_substr.c

OBJS = $(SRCS:.c=.o)

CC = gcc
CFLAGS = -Wall -Werror -Wextra

$(NAME): $(OBJS)
		ar rc $(NAME) $(OBJS)
		ranlib $(NAME)

all: $(NAME)

clean:
		rm -f $(OBJS)

fclean: clean
		rm -f $(NAME)

re: fclean all

.PHONY: all clean fclean re

